...
// src/app/api/tracccd/route.ts - ĐÃ CHUYỂN SANG MONGODB
import { NextResponse } from 'next/server';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const db = client.db(); // tự động dùng DB mặc định trong URI
    const collection = db.collection('tracuu_logs');

    // Tìm trong DB trước
    const existing = await collection.findOne({ cccd });
    if (existing && existing.ten && !existing.ten.includes('chưa có') && !existing.ten.includes('Cập nhật')) {
      return NextResponse.json({
        ten: existing.ten,
        mst: existing.mst || cccd,
        tinh: existing.tinh || tinh,
        gioitinh: existing.gioitinh || gioitinh,
        namsinh: existing.namsinh || namsinh
      });
    }

    // Nếu không có → crawl như cũ
    const delay = 800 + (parseInt(cccd.slice(-4)) % 1200);
    await new Promise(r => setTimeout(r, delay));
    ...
    // Lưu vào MongoDB (chỉ lưu khi có tên thật)
    if (finalTen !== 'CCCD chưa có mã số thuế cá nhân') {
      await collection.updateOne(
        { cccd },
        { $set: { ten: finalTen, mst: finalMst, tinh, gioitinh, namsinh, updated_at: new Date() } },
        { upsert: true }
      );
    }
    ...
    // await client.close(); // Không close để reuse connection
  }
}
